...
/*
    @brief:
    draw a rectangle in the frame buffer with the color in x10
    address base (x3, x4) (x8, x9).


    @param: x3, x4, x8, x9: coordinates of the pixel to map
    @param: x5: height and width of the square, does not modify it
    @param: w24: color of the square
    @save_stack: x3, x4, x6, x7, x30
*/
...
draw_rectangle:
    ...
    bl map
    sub x6, x9, x4		  	// height
draw_rectangle_loop1:
    sub x7, x8, x3     		// width
draw_rectangle_loop2:
    stur w24, [x0]
    add x0, x0, #4
    sub x7, x7, #1
    cbnz x7, draw_rectangle_loop2
    sub x4, x4, #1
    bl map
    sub x6, x6, #1
    cbnz x6, draw_rectangle_loop1
...
/* Funciona recorriendo el cuadrado mínimo que contiene al círculo,
   ...
   en (x1, y1) de radio r es ver si la norma de la distancia entre el punto y el centro es menor que r.
   (x - x1)^2 + (y - y1)^2 <= r^2
*/
pintarCirculo:
